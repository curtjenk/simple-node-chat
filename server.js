...
io.sockets.on('connect', function(socket) {
    ...
    for (i=0; i<MessageArr.length; i++) {
      ...
      socket.emit('message_to_client', { history: true, message: MessageArr[i].message, name: MessageArr[i].name, date: MessageArr[i].date});
    }
    if (i>0) {
    ...
    socket.on('message_to_server', function(data) {
       ...
       MessageArr.push(msg);
        ...
        io.sockets.emit('message_to_client', { message: data.message, name: data.name, date: msg.date });
    })
})
